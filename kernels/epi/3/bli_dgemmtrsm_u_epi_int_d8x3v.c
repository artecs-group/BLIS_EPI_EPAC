...
void bli_dgemmtrsm_u_epi_scalar_8x3v
     (
       dim_t                m,
       dim_t                n,
       dim_t                k0,
     ...
     )
{
	...
	for ( i = 0; i < k_iter; ++i )
	{
		// Begin iteration 0
 		...
 		bv01 = __builtin_epi_vload_1xf64( b21+1*vlen, gvl );
 		bv02 = __builtin_epi_vload_1xf64( b21+2*vlen, gvl );
		...
		abv72 = __builtin_epi_vfmacc_1xf64( abv72, sav1, bv02, gvl );
 		...
 		// Adjust pointers for next iterations.
	        //b21 += 3 * vlen * 1; // 3 vectors + 1 Unroll factor
		//a12 += 8;
	        b21 += nr;
		a12 += mr;
	}
	...
	//
	// Iteration 0  --------------------------------------------
 	__epi_1xf64 alpha77 = BROADCAST_f64( *(a11 + 7 + 7 * 8), gvl ); // (1/alpha00)

#ifdef BLIS_ENABLE_TRSM_PREINVERSION
	cv70 = __builtin_epi_vfmul_1xf64( cv70, alpha77, gvl ); // cv00 *= (1/alpha00)
	cv71 = __builtin_epi_vfmul_1xf64( cv71, alpha77, gvl ); // cv00 *= (1/alpha00)
	cv72 = __builtin_epi_vfmul_1xf64( cv72, alpha77, gvl ); // cv00 *= (1/alpha00)
#else
	cv70 = __builtin_epi_vfdiv_1xf64( cv70, alpha77, gvl ); // cv00 /= alpha00
	cv71 = __builtin_epi_vfdiv_1xf64( cv71, alpha77, gvl ); // cv00 /= alpha00
	cv72 = __builtin_epi_vfdiv_1xf64( cv72, alpha77, gvl ); // cv00 /= alpha00
#endif

	__builtin_epi_vstore_1xf64( b11 + 7 * nr + 0 * vlen * cs_c, cv70, gvl ); // Store row 1 of b11
	__builtin_epi_vstore_1xf64( b11 + 7 * nr + 1 * vlen * cs_c, cv71, gvl ); // Store row 1 of b11
	__builtin_epi_vstore_1xf64( b11 + 7 * nr + 2 * vlen * cs_c, cv72, gvl ); // Store row 1 of b11
	...
	// Iteration 1  --------------------------------------------
	//
	__epi_1xf64 alpha67 = BROADCAST_f64( *(a11 + 6 + 7 * 8), gvl ); // (alpha10)
	__epi_1xf64 alpha66 = BROADCAST_f64( *(a11 + 6 + 6 * 8), gvl ); // (1/alpha11)

	__epi_1xf64 accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha67, gvl );  // Scale row 1 of B11
	__epi_1xf64 accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha67, gvl );  // Scale row 1 of B11
	__epi_1xf64 accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha67, gvl );  // Scale row 1 of B11

	cv60  = __builtin_epi_vfsub_1xf64( cv60, accum0, gvl );    // Update row 2 of B11
	cv61  = __builtin_epi_vfsub_1xf64( cv61, accum1, gvl );    // Update row 2 of B11
	cv62  = __builtin_epi_vfsub_1xf64( cv62, accum2, gvl );    // Update row 2 of B11
	...
	__builtin_epi_vstore_1xf64( b11 + 6 * nr + 0 * vlen * cs_c, cv60, gvl ); // Store row 2 of b11
	__builtin_epi_vstore_1xf64( b11 + 6 * nr + 1 * vlen * cs_c, cv61, gvl ); // Store row 2 of b11
	__builtin_epi_vstore_1xf64( b11 + 6 * nr + 2 * vlen * cs_c, cv62, gvl ); // Store row 2 of b11
	...
	// Iteration 2  --------------------------------------------
	//
	__epi_1xf64 alpha57 = BROADCAST_f64( *(a11 + 5 + 7 * 8), gvl ); // (alpha20)
	__epi_1xf64 alpha56 = BROADCAST_f64( *(a11 + 5 + 6 * 8), gvl ); // (alpha21)
	__epi_1xf64 alpha55 = BROADCAST_f64( *(a11 + 5 + 5 * 8), gvl ); // (1/alpha22)

	accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha57, gvl );            // 
	accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha57, gvl );            // 
	accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha57, gvl );            // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha56, cv60, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha56, cv61, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha56, cv62, gvl );    // 
	...
	__builtin_epi_vstore_1xf64( b11 + 5 * nr + 0 * vlen * cs_c, cv50, gvl ); // Store row 3 of b11
	__builtin_epi_vstore_1xf64( b11 + 5 * nr + 1 * vlen * cs_c, cv51, gvl ); // Store row 3 of b11
	__builtin_epi_vstore_1xf64( b11 + 5 * nr + 2 * vlen * cs_c, cv52, gvl ); // Store row 3 of b11
	...
	// Iteration 3  --------------------------------------------
	//
	__epi_1xf64 alpha47 = BROADCAST_f64( *(a11 + 4 + 7 * 8), gvl ); // (alpha30)
	__epi_1xf64 alpha46 = BROADCAST_f64( *(a11 + 4 + 6 * 8), gvl ); // (alpha31)
	__epi_1xf64 alpha45 = BROADCAST_f64( *(a11 + 4 + 5 * 8), gvl ); // (alpha32)
	__epi_1xf64 alpha44 = BROADCAST_f64( *(a11 + 4 + 4 * 8), gvl ); // (1/alpha33)

	accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha47, gvl );            // 
	accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha47, gvl );            // 
	accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha47, gvl );            // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha46, cv60, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha46, cv61, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha46, cv62, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha45, cv50, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha45, cv51, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha45, cv52, gvl );    // 
	...
	__builtin_epi_vstore_1xf64( b11 + 4 * nr + 0 * vlen * cs_c, cv40, gvl ); // Store row 4 of b11
	__builtin_epi_vstore_1xf64( b11 + 4 * nr + 1 * vlen * cs_c, cv41, gvl ); // Store row 4 of b11
	__builtin_epi_vstore_1xf64( b11 + 4 * nr + 2 * vlen * cs_c, cv42, gvl ); // Store row 4 of b11
	...
	// Iteration 4  --------------------------------------------
	//
	__epi_1xf64 alpha37 = BROADCAST_f64( *(a11 + 3 + 7 * 8), gvl ); // (alpha40)
	__epi_1xf64 alpha36 = BROADCAST_f64( *(a11 + 3 + 6 * 8), gvl ); // (alpha41)
	__epi_1xf64 alpha35 = BROADCAST_f64( *(a11 + 3 + 5 * 8), gvl ); // (alpha42)
	__epi_1xf64 alpha34 = BROADCAST_f64( *(a11 + 3 + 4 * 8), gvl ); // (alpha43)
	__epi_1xf64 alpha33 = BROADCAST_f64( *(a11 + 3 + 3 * 8), gvl ); // (1/alpha44)

	accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha37, gvl );            // 
	accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha37, gvl );            // 
	accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha37, gvl );            // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha36, cv60, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha36, cv61, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha36, cv62, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha35, cv50, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha35, cv51, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha35, cv52, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha34, cv40, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha34, cv41, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha34, cv42, gvl );    // 
	...
	__builtin_epi_vstore_1xf64( b11 + 3 * nr + 0 * vlen * cs_c, cv30, gvl ); // Store row 5 of b11
	__builtin_epi_vstore_1xf64( b11 + 3 * nr + 1 * vlen * cs_c, cv31, gvl ); // Store row 5 of b11
	__builtin_epi_vstore_1xf64( b11 + 3 * nr + 2 * vlen * cs_c, cv32, gvl ); // Store row 5 of b11
	...
	// Iteration 5  --------------------------------------------
	//
	__epi_1xf64 alpha27 = BROADCAST_f64( *(a11 + 2 + 7 * 8), gvl ); // (alpha50)
	__epi_1xf64 alpha26 = BROADCAST_f64( *(a11 + 2 + 6 * 8), gvl ); // (alpha51)
	__epi_1xf64 alpha25 = BROADCAST_f64( *(a11 + 2 + 5 * 8), gvl ); // (alpha52)
	__epi_1xf64 alpha24 = BROADCAST_f64( *(a11 + 2 + 4 * 8), gvl ); // (alpha53)
	__epi_1xf64 alpha23 = BROADCAST_f64( *(a11 + 2 + 3 * 8), gvl ); // (alpha54)
	__epi_1xf64 alpha22 = BROADCAST_f64( *(a11 + 2 + 2 * 8), gvl ); // (1/alpha55)

	accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha27, gvl );            // 
	accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha27, gvl );            // 
	accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha27, gvl );            // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha26, cv60, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha26, cv61, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha26, cv62, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha25, cv50, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha25, cv51, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha25, cv52, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha24, cv40, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha24, cv41, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha24, cv42, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha23, cv30, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha23, cv31, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha23, cv32, gvl );    // 
	...
	__builtin_epi_vstore_1xf64( b11 + 2 * nr + 0 * vlen * cs_c, cv20, gvl ); // Store row 6 of b11
	__builtin_epi_vstore_1xf64( b11 + 2 * nr + 1 * vlen * cs_c, cv21, gvl ); // Store row 6 of b11
	__builtin_epi_vstore_1xf64( b11 + 2 * nr + 2 * vlen * cs_c, cv22, gvl ); // Store row 6 of b11
	...
	// Iteration 6  --------------------------------------------
	//
	__epi_1xf64 alpha17 = BROADCAST_f64( *(a11 + 1 + 7 * 8), gvl ); // (alpha60)
	__epi_1xf64 alpha16 = BROADCAST_f64( *(a11 + 1 + 6 * 8), gvl ); // (alpha61)
	__epi_1xf64 alpha15 = BROADCAST_f64( *(a11 + 1 + 5 * 8), gvl ); // (alpha62)
	__epi_1xf64 alpha14 = BROADCAST_f64( *(a11 + 1 + 4 * 8), gvl ); // (alpha63)
	__epi_1xf64 alpha13 = BROADCAST_f64( *(a11 + 1 + 3 * 8), gvl ); // (alpha64)
	__epi_1xf64 alpha12 = BROADCAST_f64( *(a11 + 1 + 2 * 8), gvl ); // (alpha65)
	__epi_1xf64 alpha11 = BROADCAST_f64( *(a11 + 1 + 1 * 8), gvl ); // (1/alpha66)

	accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha17, gvl );            // 
	accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha17, gvl );            // 
	accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha17, gvl );            // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha16, cv60, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha16, cv61, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha16, cv62, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha15, cv50, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha15, cv51, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha15, cv52, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha14, cv40, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha14, cv41, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha14, cv42, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha13, cv30, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha13, cv31, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha13, cv32, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha12, cv20, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha12, cv21, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha12, cv22, gvl );    // 

	cv10 = __builtin_epi_vfsub_1xf64( cv10, accum0, gvl );    // 
	cv11 = __builtin_epi_vfsub_1xf64( cv11, accum1, gvl );    // 
	cv12 = __builtin_epi_vfsub_1xf64( cv12, accum2, gvl );    // 
	...
	__builtin_epi_vstore_1xf64( b11 + 1 * nr + 0 * vlen * cs_c, cv10, gvl ); // Store row 7 of b11
	__builtin_epi_vstore_1xf64( b11 + 1 * nr + 1 * vlen * cs_c, cv11, gvl ); // Store row 7 of b11
	__builtin_epi_vstore_1xf64( b11 + 1 * nr + 2 * vlen * cs_c, cv12, gvl ); // Store row 7 of b11
	...
	// Iteration 7  --------------------------------------------
	//
	__epi_1xf64 alpha07 = BROADCAST_f64( *(a11 + 0 + 7 * 8), gvl ); // (alpha70)
	__epi_1xf64 alpha06 = BROADCAST_f64( *(a11 + 0 + 6 * 8), gvl ); // (alpha71)
	__epi_1xf64 alpha05 = BROADCAST_f64( *(a11 + 0 + 5 * 8), gvl ); // (alpha72)
	__epi_1xf64 alpha04 = BROADCAST_f64( *(a11 + 0 + 4 * 8), gvl ); // (alpha73)
	__epi_1xf64 alpha03 = BROADCAST_f64( *(a11 + 0 + 3 * 8), gvl ); // (alpha74)
	__epi_1xf64 alpha02 = BROADCAST_f64( *(a11 + 0 + 2 * 8), gvl ); // (alpha75)
	__epi_1xf64 alpha01 = BROADCAST_f64( *(a11 + 0 + 1 * 8), gvl ); // (alpha76)
	__epi_1xf64 alpha00 = BROADCAST_f64( *(a11 + 0 + 0 * 8), gvl ); // (1/alpha77)

	accum0 = __builtin_epi_vfmul_1xf64( cv70, alpha07, gvl );            // 
	accum1 = __builtin_epi_vfmul_1xf64( cv71, alpha07, gvl );            // 
	accum2 = __builtin_epi_vfmul_1xf64( cv72, alpha07, gvl );            // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha06, cv60, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha06, cv61, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha06, cv62, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha05, cv50, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha05, cv51, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha05, cv52, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha04, cv40, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha04, cv41, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha04, cv42, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha03, cv30, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha03, cv31, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha03, cv32, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha02, cv20, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha02, cv21, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha02, cv22, gvl );    // 

	accum0 = __builtin_epi_vfmacc_1xf64( accum0, alpha01, cv10, gvl );    // 
	accum1 = __builtin_epi_vfmacc_1xf64( accum1, alpha01, cv11, gvl );    // 
	accum2 = __builtin_epi_vfmacc_1xf64( accum2, alpha01, cv12, gvl );    // 

	cv00 = __builtin_epi_vfsub_1xf64( cv00, accum0, gvl );    // 
	cv01 = __builtin_epi_vfsub_1xf64( cv01, accum1, gvl );    // 
	cv02 = __builtin_epi_vfsub_1xf64( cv02, accum2, gvl );    // 

#ifdef BLIS_ENABLE_TRSM_PREINVERSION
	cv00 = __builtin_epi_vfmul_1xf64( cv00, alpha00, gvl ); // abv0 *= alpha00
	cv01 = __builtin_epi_vfmul_1xf64( cv01, alpha00, gvl ); // abv0 *= alpha00
	cv02 = __builtin_epi_vfmul_1xf64( cv02, alpha00, gvl ); // abv0 *= alpha00
#else
	cv00 = __builtin_epi_vfdiv_1xf64( cv00, alpha00, gvl ); // abv0 *= alpha00
	cv01 = __builtin_epi_vfdiv_1xf64( cv01, alpha00, gvl ); // abv0 *= alpha00
	cv02 = __builtin_epi_vfdiv_1xf64( cv02, alpha00, gvl ); // abv0 *= alpha00
#endif

	__builtin_epi_vstore_1xf64( b11 + 0 * nr + 0 * vlen * cs_c, cv00, gvl ); // Store row 8 of b11
	__builtin_epi_vstore_1xf64( b11 + 0 * nr + 1 * vlen * cs_c, cv01, gvl ); // Store row 8 of b11
	__builtin_epi_vstore_1xf64( b11 + 0 * nr + 2 * vlen * cs_c, cv02, gvl ); // Store row 8 of b11
